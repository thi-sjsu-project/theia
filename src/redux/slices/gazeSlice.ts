...
export const gazeSlice = createSlice({
  ...
  reducers: {
    ...
    setElementsInGaze: (state, action: PayloadAction<ElementInGaze[]>) => {
      ...
      oldElements.forEach(function (oldElement, oldElementIndex) {
        //find the elements that were previously in gaze and still are. effectively removes the elements that are no longer in gaze.
        if (elementIds.includes(oldElement.id)) {
          //the element is still in gaze, add it
          elementsInGaze.push(oldElement);
          elementsInGazeIds.push(oldElement.id);
        }
      });

      elements.forEach(function (element, elementIdIndex) {
        //find if elements aren't in the state yet, and add them if they are not in state
        if (!elementsInGazeIds.includes(element.id)) {
          //element is not in the state yet, so add it
