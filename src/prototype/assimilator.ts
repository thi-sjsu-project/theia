import type { VehicleWidget, Widget, WidgetMap } from 'src/types/widget';
...
const assimilator = ({
  ...
  // If there are no possible widget clusters and no message, then this is the initial setup
  if (!possibleWidgetClusters && !message) {
    let ownshipWidget: VehicleWidget = {} as VehicleWidget;
    let droneWidgets: VehicleWidget[] = [];
    let newWidgets: WidgetMap = {};
    Object.values(deployedWidgets).forEach((widget: any) => {
      if (widget.type === 'vehicle') {
        let newWidget = {...widget};
        const minimapSection: any = sections.find(
          (section) => section.type === 'vehicle',
        )!;

        // For each widget in the deployedWidgets, check if it has an ownship tag or drone tag
        // Also, set the x and y coordinates of the widget to a random position within the minimap section
        if (newWidget.tags.includes('ownship')) {
          newWidget.x = Math.floor(Math.random() * (minimapSection.w - 400) + 200);
          newWidget.y = Math.floor(Math.random() * (minimapSection.h - 400) + 200);
          ownshipWidget = newWidget;
          newWidgets[widget.id] = newWidget;
        } else if (newWidget.tags!.includes('drone')) {
          newWidget.x = Math.floor(Math.random() * (minimapSection.w - 400) + 200);
          newWidget.y = Math.floor(Math.random() * (minimapSection.h - 400) + 200);
          // Add specific tags to drone widgets
          const newTag = 'aca' + widget.vehicleId;
          newWidget.tags = [...newWidget.tags, newTag];
          droneWidgets.push(newWidget);
          newWidgets[widget.id] = newWidget;
        }
      }
    });
    return { ownshipWidget, droneWidgets, newWidgets };
  }
    ...
    widgetCluster.widgets.forEach((widget, widgetIndex) => {
      ...
      if (
        ...
        matchingSections.forEach(function (section, sectionIndex) {
          ...
          if (Object.prototype.hasOwnProperty.call(message, 'target')) {
            // @ts-ignore
            if (
              // @ts-ignore
