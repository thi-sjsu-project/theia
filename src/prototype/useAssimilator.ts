 ...
 * @returns ???
 */
const useAssimilator = ({possibleWidgets}: AssimilatorProps) => {

  const grid = store.getState().cm.grid


  let widgetToDeploy = null;//will return null if we cannot find a space
  grid.forEach(function(row,rowIndex){ //go through each row
    row.forEach(function(widget, colIndex) { //go through each widget in row
      if(widget == null){ //found space to put the widget in
